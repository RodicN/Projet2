            ...
            <?php if (count($sessions) > 0): ?>
                <?php foreach ($sessions as $row): ?>
                    <tr>
                        ...
                        <td>
                            <button onclick="if(confirm('Êtes-vous sûr de vouloir vous inscrire à cette session?')) location.href='../CONTROLLEUR/inscriptionSession.php?idSession=<?= $row['ID_Session'] ?>'">Inscription</button>
                        </td>
                    </tr>
                <?php endforeach; ?>
            <?php else: ?>
